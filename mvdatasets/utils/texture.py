...
class RGBATexture:
    def __init__(
        ...
        assert texture_path.endswith(".png"), f"texture {texture_path} must be a .png file"
        if verbose:
            print(f"[bold blue]INFO[/bold blue]: loading texture {texture_path.split('/')[-1]}")
        ...
        if verbose:
            print(f"[bold blue]INFO[/bold blue]: texture {texture_path.split('/')[-1]} loaded: {texture.shape}")
        self.image = texture
...
class SHTexture:
    def __init__(
        ...
        for texture_meta in textures_meta:
            ...
            nr_read_sh_coeffs += 4
            if verbose:
                print(f"[bold blue]INFO[/bold blue]: texture {texture_path.split('/')[-1]} loaded: {sh_coeffs.shape}")
        ...
        if verbose:
            print(f"[bold blue]INFO[/bold blue]: sh coeffs params {texture.shape} loaded")
        self.image = texture
