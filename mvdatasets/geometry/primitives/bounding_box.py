...
class BoundingBox:
    ...
    def __init__(
        ...
        if father_bb is not None:
            if self.father_bb.device != device:
                raise ValueError(
                    "father and child and bounding boxes must be on the same device"
                )

        if self.father_bb is None:
            self.local_scale = torch.tensor(
                local_scale, dtype=torch.float32, device=device
            )
        else:
        ...
        if verbose:
            print(
                f"created bounding box with local sides lengths: {self.local_scale.cpu().numpy()}"
            )
    ...
    def intersect(self, rays_o, rays_d):
        ...
        is_hit, t_near, t_far = _intersect_aabb(rays_o, rays_d, aabb_min, aabb_max)
    ...
    def get_random_points_inside(self, nr_points, in_world_space=False, padding=0.0):
        ...
        points = (
            torch.rand((nr_points, 3), dtype=torch.float32, device=self.device) * 2.0
            - 1.0
        )
        points = points * scale
    ...
    def get_random_points_on_surface(self, nr_points, in_world_space=False):
        ...
        bar_coords = torch.rand(
            (nr_points_, 3), dtype=torch.float32, device=self.device
        )
        ...
        multipled_tensors = (
            tri_vertices_reshaped * bar_coords_reshaped
        )  # (12, nr_points_per_face, 3, 3)
