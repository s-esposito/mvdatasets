...
def load_data(data_path, H=800, W=800):
    ...
    for entry in tqdm(data_info):
        ...
        img = np.array(imageio.imread(os.path.join(data_path, entry["file_path"])))[
            ..., :3
        ]

        cv.imwrite(
            os.path.join(data_path, f"data/m_{cam_id}_{frame_id}.png"), img[..., ::-1]
        )
        rgb_all[cam_id, frame_id] = torch.from_numpy(img.astype(np.float32) / 255.0)
