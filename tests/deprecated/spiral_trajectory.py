...
class SphereSpiral:
    ...
    def create_points_trajectory(self):
        ...
        p = self._spiral(
                    azimuth_deg=np.tile(np.linspace(1, 360, self.points_per_turn), self.turns), # angle around the z-axis
                    elevation_deg=np.linspace(-90, 90, self.turns * self.points_per_turn) # angle from the xy-plane
                )
